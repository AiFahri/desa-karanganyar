            ...
            <div ref={formRef}>
              ...
              <form onSubmit={handleSubmit} className="flex flex-wrap gap-10">
                <div className="flex flex-col gap-6 w-[500px]">
                  <div>
                    <label className="text-[18px] font-semibold mb-2 block">Jumlah Penduduk</label>
                    <input
                      type="number"
                      value={data.jumlah_penduduk}
                      onChange={(e) => setData('jumlah_penduduk', parseInt(e.target.value) || 0)}
                    ...
                    />
                    {errors.jumlah_penduduk && <p className="text-red-500 text-sm mt-1">{errors.jumlah_penduduk}</p>}
                  </div>

                  <div>
                    <label className="text-[18px] font-semibold mb-2 block">Jumlah Kepala Keluarga</label>
                    <input
                      type="number"
                      value={data.jumlah_kepala_keluarga}
                      onChange={(e) => setData('jumlah_kepala_keluarga', parseInt(e.target.value) || 0)}
                      placeholder="Masukkan jumlah kepala keluarga"
                      className="w-full h-[50px] px-5 py-3 border border-[#D9D9D9] bg-[#FDFCFC] rounded-lg text-sm font-semibold text-black placeholder:text-black/45"
                    />
                    {errors.jumlah_kepala_keluarga && <p className="text-red-500 text-sm mt-1">{errors.jumlah_kepala_keluarga}</p>}
                  </div>

                  <div>
                    <label className="text-[18px] font-semibold mb-2 block">Jumlah Pria</label>
                    <input
                      type="number"
                      value={data.jumlah_pria}
                      onChange={(e) => setData('jumlah_pria', parseInt(e.target.value) || 0)}
                      placeholder="Masukkan jumlah pria"
                      className="w-full h-[50px] px-5 py-3 border border-[#D9D9D9] bg-[#FDFCFC] rounded-lg text-sm font-semibold text-black placeholder:text-black/45"
                    />
                    {errors.jumlah_pria && <p className="text-red-500 text-sm mt-1">{errors.jumlah_pria}</p>}
                  </div>

                  <div>
                    <label className="text-[18px] font-semibold mb-2 block">Jumlah Wanita</label>
                    <input
                      type="number"
                      value={data.jumlah_wanita}
                      onChange={(e) => setData('jumlah_wanita', parseInt(e.target.value) || 0)}
                      placeholder="Masukkan jumlah wanita"
                      className="w-full h-[50px] px-5 py-3 border border-[#D9D9D9] bg-[#FDFCFC] rounded-lg text-sm font-semibold text-black placeholder:text-black/45"
                    />
                    {errors.jumlah_wanita && <p className="text-red-500 text-sm mt-1">{errors.jumlah_wanita}</p>}
                  </div>
                </div>
                ...
                <div className="flex flex-col w-[400px] justify-center items-center">
                  <div className="text-center p-8 bg-gray-50 rounded-lg">
                    ...
                    <div className="space-y-2 text-sm">
                      <p><strong>Total Penduduk:</strong> {data.jumlah_penduduk.toLocaleString('id-ID')}</p>
                      <p><strong>Kepala Keluarga:</strong> {data.jumlah_kepala_keluarga.toLocaleString('id-ID')}</p>
                      <p><strong>Pria:</strong> {data.jumlah_pria.toLocaleString('id-ID')}</p>
                      <p><strong>Wanita:</strong> {data.jumlah_wanita.toLocaleString('id-ID')}</p>
                      <div className="mt-4 pt-4 border-t">
                        <p className={`text-sm ${(data.jumlah_pria + data.jumlah_wanita) === data.jumlah_penduduk ? 'text-green-600' : 'text-red-600'}`}>
                          Validasi: {(data.jumlah_pria + data.jumlah_wanita) === data.jumlah_penduduk ? '✓ Valid' : '✗ Pria + Wanita ≠ Total'}
                        </p>
                      </div>
                    </div>
                  </div>
